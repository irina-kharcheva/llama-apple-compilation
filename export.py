...
class StatefulLlamaForCausalLM(torch.nn.Module):
    def __init__(self, model_path: str, max_context_size: int = 2048, batch_size: int = 1) -> None:
        ...
        config_dict, _ = LlamaConfig.get_config_dict(model_path, token=True)
        config_dict["rope_scaling"] = {"type": "linear", "factor": 2.0}
        model_config = LlamaConfig.from_dict(config_dict)
        self.model = LlamaForCausalLM.from_pretrained(model_path, config=model_config, token=True)
...
def export() -> None:
    ...
    mlmodel_fp16.save("StatefulLlama1BInstruct.mlpackage")
